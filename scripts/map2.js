//$(window).on('load', function() {
  ...
  function determineLayers(points) {
    ...
    if (layerNamesFromSpreadsheet.length === 1) {
      ...
      for (var i in layerNamesFromSpreadsheet) {
        ...
        if (layerNameFromSpreadsheet != 'Indivisible' &&  layerNameFromSpreadsheet != 'NAACP'){
  ...
  function mapPoints(points, layers) {
    ...
    for (var i in points) {
      ...
      // The is the old marker and popup creation   
      ...
      var colorX = point['Marker Color'].toLowerCase();
      if (point.Latitude !== '' && point.Longitude !== '') {
        var marker = L.circleMarker([point.Latitude, point.Longitude], {
            color: '#fff',
            stroke: false,
            weight: 1,
            fillColor: point['Marker Color'].toLowerCase(),
            fillOpacity: 0.4,
            radius: 14
          })
          .bindPopup('<p><b>' + point['Group'] + '</b></p><p>'  + point['City'] + ', '  + point['State'] + '</p><p>' + point['Title'] + '</p><p>' + point['Description'] + '</p><p><a href=\"' + point['Image'] + '\" target="_blank">' +  point['Image'] + '</a>' );
    ...
    if (layers === undefined || layers.length === 0) {
      ...
      if (getSetting('_pointsLegendPos') !== 'off') {
        ...
        // Move legend into sidebar
        pointsLegend._container.remove();
        document.getElementById('home').appendChild(pointsLegend.onAdd(map));

        pointsLegend._container.id = 'points-legend-off';
      }
    }
    ...
    $('#points-legend-off').prepend('<h6 class="pointer">' + getSetting('_pointsLegendTitle') + '</h6>');
  ...
  function processPolygons() {
    ...
    polygonsLegend._container.remove();
    document.getElementById('home').appendChild(polygonsLegend.onAdd(map));
  ...
  function onEachFeature(feature, layer) {
    ...
    //var pxlabel = new L.Label();

    // Add polygon label if needed //html: feature.properties[getSetting('_polygonLabel')],
  ...
  function onMapDataLoad() {
    ...
    map.createPane('pgons');
    map.getPane('pgons').style.zIndex = 250;
    ...
    // document.getElementsByClassName('leaflet-pelias-control')[0].setAttribute('class', 'leaflet-pelias-control leaflet-bar leaflet-control leaflet-pelias-expanded');
    // document.getElementsByClassName('leaflet-pelias-input')[0].setAttribute('placeholder', 'search locations');

    // Generate icons for markers legend
    $('#points-legend-off form label span').each(function(i) {
  ...
  // <h1 class="sidebar-header">Resistance Map<span class="sidebar-close"><i class="fa fa-caret-left"></i></span></h1>

  // function addTitle() {
  //   var dispTitle = getSetting('_mapTitleDisplay');

  //   if (dispTitle !== 'off') {
  //     var title = getSetting('_mapTitle');
  //     var subtitle = getSetting('_mapSubtitle');

  //     if (dispTitle == 'on') {
  //       //$('div.sidebar-pane').prepend('<h1 class="sidebar-header">' + title + ' ' + subtitle + '<span class="sidebar-close"><i class="fa fa-caret-left"></i></span></h1>');
   ...
   */
  // function addBaseMap() {
  //   var basemap = trySetting('_tileProvider', 'CartoDB.light_all');
